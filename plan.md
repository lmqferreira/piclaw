...
- Backend size: **164 TS files / 22,495 LOC** (`src/`)
- Frontend size: **7,095 LOC** (`web/src/`)
- Tests: **591 passing, 0 failing**
- Lint: passing (for current backend tranche)
- Coverage (line): **57.97%** (`coverage/lcov.info`)
- Review comment coverage: Added focused regression/unit tests for each recent extraction seam (`web/recovery.ts`, `web/agent-buffers.ts`, `web/auth-runtime.ts`, `web/agent-status-store.ts`, `web/pending-steering.ts`, `web/interaction-broadcaster.ts`, `web/followup-placeholders.ts`, runtime wiring/provider bootstrap) so refactors remain behavior-preserving.
