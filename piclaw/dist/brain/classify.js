...
async function getCopilotToken() {
    ...
    if (!res.ok)
        throw new Error(`Copilot token error: ${res.status}`);
...
export async function resolveEntryByQuery(query, entries) {
    if (entries.length === 0)
...
Return ONLY valid JSON, no explanation.`,
    ...
    if (!res.ok)
        throw new Error(`AI resolve failed: ${res.status}`);
    const data = (await res.json());
