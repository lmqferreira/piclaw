 ...
 */
export function insertWebSession(token, expiresAt) {
    const db = getDb();
 ...
 */
export function pruneExpiredWebSessions(nowMs = Date.now()) {
    const db = getDb();
 ...
 */
export function loadActiveWebSessions(nowMs = Date.now()) {
    const db = getDb();
    const rows = db.prepare("SELECT token, expires_at FROM web_sessions WHERE expires_at > ?").all(nowMs);
    const map = new Map();
