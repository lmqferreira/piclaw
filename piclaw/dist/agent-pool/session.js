...
export async function createDefaultSession(chatJid, options) {
    ...
    const preferredProvider = options.settingsManager.getDefaultProvider?.() ?? options.settingsManager.settings?.defaultProvider;
    const preferredModel = options.settingsManager.getDefaultModel?.() ?? options.settingsManager.settings?.defaultModel;
