...
import { insertEntry, getRecentEntries, getCategoryStats, markEntryDone, markEntryArchived, findEntriesByName, getAllActiveEntries, getDuplicateGroups, archiveDuplicates, writeUndoLog, applyUndo, updateEntryCategory, getEntryById, } from "../../brain/db.js";
...
// ── /capture ──────────────────────────────────────────────────────────────────
export async function handleCapture(session, command) {
    ...
    const fieldLines = [];
    if (f.context)
        fieldLines.push(`  → Context: ${f.context}`);
    if (f.follow_up)
        fieldLines.push(`  → Follow-up: ${f.follow_up}`);
    if (f.next_action)
        fieldLines.push(`  → Next: ${f.next_action}`);
    if (f.status)
        fieldLines.push(`  → Status: ${f.status}`);
    if (f.one_liner)
        fieldLines.push(`  → ${f.one_liner}`);
    if (f.due)
        fieldLines.push(`  → Due: ${f.due}`);
    if (f.notes)
        fieldLines.push(`  → Notes: ${f.notes}`);
...
// ── /sb ───────────────────────────────────────────────────────────────────────
export async function handleSb(_session, command) {
    ...
    if (!sub) {
        ...
        if (categoryMatch) {
            ...
            for (const entry of entries.slice(0, 25)) {
                ...
                const f = typeof entry.fields === "string" ? JSON.parse(entry.fields) : entry.fields;
                ...
                const tags = [];
                if (isDup)
                    tags.push(`⚠️ ×${dupCount} duplicate`);
                if (noNextAction)
                    tags.push("⚠️ no next action");
                if (isStale && !isDup)
                    tags.push(`🕐 ${ageDays}d`);
                lines.push(`${CATEGORY_EMOJI[entry.category] ?? "•"} **${entry.name}**${tags.length ? " " + tags.join(" ") : ""}`);
                if (f.next_action)
                    lines.push(`  → Next: ${f.next_action}`);
                else if (f.notes)
                    lines.push(`  → ${f.notes.slice(0, 80)}`);
                const suggestions = [];
                if (isDup)
                    suggestions.push(`/sb fix dup ${categoryMatch}`);
                else if (isStale)
                    suggestions.push(`/sb do ${entry.name.toLowerCase().slice(0, 30)}  |  /sb ar ${entry.name.toLowerCase().slice(0, 30)}`);
                if (noNextAction)
                    suggestions.push(`/capture next action for ${entry.name.toLowerCase().slice(0, 30)}: [what]`);
    ...
    if (sub === "ls" || sub === "list") {
        ...
        if (sub === "list" && !category) {
            ...
            const grouped = { people: [], projects: [], ideas: [], admin: [] };
        ...
        for (const entry of entries.slice(0, 25)) {
            ...
            if (f.next_action)
                lines.push(`  → Next: ${f.next_action}`);
            if (f.notes && !f.next_action)
                lines.push(`  → ${f.notes.slice(0, 80)}`);
            // Suggested actions
            const suggestions = [];
            if (isDup)
                suggestions.push(`/sb fix dup${category ? " " + entry.category : ""}`);
            else if (isStale)
                suggestions.push(`/sb do ${entry.name.toLowerCase().slice(0, 30)}  |  /sb ar ${entry.name.toLowerCase().slice(0, 30)}`);
            if (noNextAction)
                suggestions.push(`/capture next action for ${entry.name.toLowerCase().slice(0, 30)}: [what]`);
    ...
    if (sub === "stats") {
        ...
        if (total === 0)
            return { status: "success", message: "No entries yet. Use /capture <thought> to start." };
        ...
        lines.push(`\nTotal: ${total}`);
        if (dups > 0)
            lines.push(`⚠️ ${dups} duplicate entries → /sb fix dup`);
        return { status: "success", message: lines.join("\n") };
    }
    // ── undo ─────────────────────────────────────────────────────────────────────
    if (sub === "undo") {
        ...
        if (fixSub.startsWith("cat")) {
            ...
            if (likeMatches.length === 1) {
                ...
                try {
                    ...
                    if (likeMatches.length > 1) {
                        const list = likeMatches.slice(0, 5).map((e) => `• [${e.category}] ${e.name}`).join("\n");
    ...
    if (sub === "show") {
        ...
        if (!entry && likeMatches.length > 1) {
            const uniqueNames = new Set(likeMatches.map((e) => e.name.toLowerCase()));
        ...
        if (likeMatches.length > 1) {
            ...
            const nameMatches = likeMatches.filter((e) => e.name.toLowerCase().includes(subArgs.toLowerCase()));
    ...
    if (sub === "nudge") {
        try {
            const proc = Bun.spawnSync(["bun", "run", "/workspace/scripts/brain-digest.ts"], { cwd: "/home/azureuser/piclaw/piclaw", env: process.env });
            if (proc.exitCode !== 0) {
    ...
    if (sub === "done" || sub === "archive") {
        ...
        if (likeMatches.length > 1) {
            const uniqueNames = new Set(likeMatches.map((e) => e.name.toLowerCase()));
            if (uniqueNames.size === 1) {
                const prevStates = likeMatches.map((e) => ({ id: e.id, status: e.status }));
                writeUndoLog(sub, likeMatches.map((e) => e.id), prevStates);
                let count = 0;
